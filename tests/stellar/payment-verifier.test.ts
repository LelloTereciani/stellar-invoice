...
describe("ledger payment verification", () => {
  ...
  it("rejects a wrong memo, amount, asset, destination, or source", () => {
    for (const candidate of [
      [ { ...transaction, memo: "other" }, operation ], [ transaction, { ...operation, amount: "9" } ], [ transaction, { ...operation, asset_code: "USDC" } ], [ transaction, { ...operation, destination: "GOTHER" } ], [ { ...transaction, source_account: "GOTHER" }, operation ], [ { ...transaction, successful: false }, operation ],
