...
export type InvoiceDraft = InvoiceInput & { assetCode: "BRLT"; assetIssuer: string; issuerPublicKey: string; memo: string; receiverPublicKey: string; status: "pending" };
...
export function createInvoiceDraft(input: InvoiceInput, issuerPublicKey: string, receiverPublicKey: string, now = new Date()): InvoiceDraft {
  ...
  return { ...input, assetCode: "BRLT", assetIssuer: issuerPublicKey, issuerPublicKey, memo: `inv-${crypto.randomUUID().replaceAll("-", "").slice(0, 24)}`, receiverPublicKey, status: "pending" };
}
