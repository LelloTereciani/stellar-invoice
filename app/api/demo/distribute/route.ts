...
import { acquireDemoDistributionLock, completeDemoDistribution, ensureDemoInvoice, getPersistentDemoSessionWallet, releaseDemoDistributionLock, reserveDemoDistribution, resetExpiredDemoDistribution, storePreparedDemoDistribution } from "../../../lib/demo/persistent-session.js";
import { DEMO_ASSET_AMOUNT, demoDistributionExists, demoDistributionExpired, prepareDemoBrltDistribution, submitPreparedDemoDistribution } from "../../../lib/demo/provisioning.js";
...
export async function POST(request: Request) {
  try {
    ...
    await acquireDemoDistributionLock(attemptKey);
    try {
      let distribution = await reserveDemoDistribution(sessionId, attemptKey);
      if (distribution.status === "prepared" && distribution.signedXdr && distribution.transactionHash) {
        const exists = await demoDistributionExists(distribution.transactionHash);
        if (exists) {
          distribution = await completeDemoDistribution(customerPublicKey, distribution.transactionHash);
        } else if (demoDistributionExpired(distribution.signedXdr)) {
          distribution = await resetExpiredDemoDistribution(customerPublicKey, distribution.transactionHash, attemptKey);
        }
      }
