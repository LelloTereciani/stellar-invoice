...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as { amount?: string; challenge?: string; challengeExpiresAt?: string; challengeId?: string; debtorPublicKey?: string; dueAt?: string; receiverPublicKey?: string; signedChallenge?: string };
    if (!body.challengeId || !body.challenge || !body.challengeExpiresAt || !body.signedChallenge || !body.debtorPublicKey || !body.amount || !body.dueAt || !body.receiverPublicKey) throw new Error("Incomplete invoice request");
    const stellar = loadStellarConfig(process.env);
    await verifyAndConsumeIssuerChallenge({
      expiresAt: body.challengeExpiresAt,
      id: body.challengeId,
      invoice: { amount: body.amount, debtorPublicKey: body.debtorPublicKey, dueAt: body.dueAt, receiverPublicKey: body.receiverPublicKey },
